...
function ShellInABox(wsurl, container) {
  //if (url == undefined) {
...
ShellInABox.prototype.keepalive = function() {
  that = this;
  if (this.sock && this.sock.readyState == 1) {
    this.sock.send(JSON.stringify({"cmd":"ping"}));


  this.keepalive_to = setTimeout(function(){that.keepalive();}, 5000);
  }
}
...
ShellInABox.prototype.sendRequest = function(request) {
  ...
  var that = this;
    this.sock.onopen = function() {
      console.log("connected to " + that.wsurl);
       setTimeout(function(shellInABox) {
        return function() {
          if (that.sock && that.sock.readyState == 1) {
           that.sock.send(JSON.stringify({"cmd":"window-change","width":that.terminalWidth, "height": that.terminalHeight}));
       }
        };
      }(that), 100);
       that.keepalive_to = setTimeout(function(){that.keepalive();}, 5000);
  }
  ...
  this.sock.onmessage = function(e) {
    clearTimeout(that.keepalive_to);
    that.keepalive_to = setTimeout(function(){that.keepalive();}, 5000);
    console.log("message received: " + e.data);
    resp =  JSON.parse(e.data);
    switch(resp.cmd) {
      case "output": {
        that.vt100(resp.params.content);
        break;
      }
      case "notify": {
        that.vt100(resp.params.message);
        break;
      }
    }
  }
...
ShellInABox.prototype.keysPressed = function(ch) {
  this.sock.send(JSON.stringify({"cmd":"shell","input":ch}));
...
ShellInABox.prototype.resized = function(w, h) {
  ...
  //if (this.session) {
    ...
    if (this.sock && this.sock.readyState == 1) {
      this.sock.send(JSON.stringify({"cmd":"window-change","width":this.terminalWidth, "height": this.terminalHeight}));
  }
  //}
};
